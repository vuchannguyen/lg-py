...
namespace Weedon
{
    public partial class FormExportExcel : Form
    {
        ...
        private void ExportKTCL(DateTime date)
        {
            ExportExcel.InitWorkBook(sheetName);
            ExportExcel.CreateSummaryKTCL(date);
            ExportExcel.CreateDetailsTableKTCL(lvEx);
        }
            ...
            if (path != null)
            {
                try
                {
                    ...
                    if (MessageBox.Show(Constant.MESSAGE_SUCCESS_EXPORT_EXCEL + Constant.MESSAGE_NEW_LINE + Constant.MESSAGE_SUCCESS_EXPORT_EXCEL_OPEN,
                        Constant.CAPTION_WARNING, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                    {
                        Process.Start(path);
                    }
                }
