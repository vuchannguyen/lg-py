...
namespace CRM.Models
{
    public class TrainingEmpEnglishInfoDao: BaseDao
    {
        ...
        public Training_EmpEnglishInfo GetListByEmpIdAndScoreType(string empId, int scoreType)
        {
            return dbContext.Training_EmpEnglishInfos.Where(p => p.EmployeeId == empId && p.TypeId == scoreType && !p.DeleteFlag && System.DateTime.Now <= p.ExpireDate).FirstOrDefault<Training_EmpEnglishInfo>();
        }
        ...
        public Training_EmpEnglishInfo GetVerbalToeicWithEmployee(string empID)
        {
            return dbContext.Training_EmpEnglishInfos.Where(q => q.EmployeeId == empID && q.TypeId == Constants.TRAINING_CENTER_SKILL_TYPE_TOEIC_VERBAL &&
                !q.DeleteFlag && (q.ExpireDate == null || (q.ExpireDate != null && q.ExpireDate.Value >= DateTime.Now))).FirstOrDefault();
        }
