...
namespace CRM.Models
{
    ...
    public class ExamQuestionDAO : BaseDao
    {
        ...
        #region New paging
        public IQueryable<LOT_ExamQuestion> GetQueryList(string name)
        {
            var sql = from data in dbContext.LOT_ExamQuestions
                      select data;

            if (!string.IsNullOrEmpty(name))
            {
                name = CommonFunc.GetFilterText(name);
                sql = sql.Where(p => SqlMethods.Like(p.Title, name)
        ...
        public int GetCountListLinq(string name)
        {
            return GetQueryList(name).Count();
        }

        public List<LOT_ExamQuestion> GetListLinq(string name,
            string sortColumn, string sortOrder, int skip, int take)
        {
            ...
            switch (sortColumn)
            {
                ...
                case "Time":
                    sortSQL += "ExamQuestionTime " + sortOrder;
                    break;
            ...
            var sql = GetQueryList(name).OrderBy(sortSQL);
                ...
                if (examQuestion != null)
                {
                    ...
                    if (id != 0)
                    {
                        ...
                        foreach (LOT_ExamQuestion_Section item in list)
                        {
                            ...
                            dbContext.LOT_ExamQuestion_Sections.InsertOnSubmit(item);
                            dbContext.SubmitChanges();
