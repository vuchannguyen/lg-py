...
namespace CRM.Models
{
    public class AuthenticateDao : BaseDao
    {
        ...
        /// <returns></returns>
        public List<GroupPermission> GetListPermissionOfUser(int userId)
        {
            List<int> groupIds =
                (from ug in dbContext.User_Groups
                where ug.IsActive
                select ug.GroupId).ToList();
            var sql =
                from gp in dbContext.GroupPermissions
                where groupIds.Contains(gp.GroupId) 
                select gp;
            return sql.ToList();
        }
