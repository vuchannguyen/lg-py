...
%>
    ...
    <div id="cactionbutton">
        <table style="width: 100%">
            <tr>
                ...
                <td align="right">
                    <button onclick="javascript:CRM.popup('/PurchaseRequest/PrintReview/?ids=<%=request.ID %>&page=','Print Review', 890);" title="Print" class="button print">Print</button>
                    <button onclick="javascript:CRM.popup('/PurchaseRequest/SendAnEmail/?ids=<%=request.ID %>&page=','Send mail', 860);" title="Send mail" class="button sendmail">Send mail</button>
                    <%
    ...
    if (request.Priority.HasValue)
    {
        ...
        Response.Write("<b style='color:red'>" + sPriority + "</b>");
            ...
            <tr>
                ...
                <td class="input" style="vertical-align:top; padding-top:2px;">
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td align="left" class="input bold" style="width: 70%; color: #0066CC; font-size: 1.4em">
                    Item(s)
                </td>
    ...
    foreach (string item in array)
    {
        ...
        z++;
                ...
                <tr>
                    ...
                    <td class="label last_right">
                        <%=CommonFunc.FormatCurrency(Math.Round(request.SaleTaxValue !=null? request.SaleTaxValue.Value:0, Constants.ROUND_NUMBER))%>
                    </td>
                </tr>
                <tr>
                    <td class="label">
    ...
    i++;
            ...
            <h2 class="heading">
                Post new comment</h2>
            <%using (Html.BeginForm("AddComment", "PurchaseRequest", FormMethod.Post, new { id = "addForm", @class = "form", enctype = "multipart/form-data" }))
              {%>
        ...
        $(document).ready(function () {
            ...
            $('#btnFirst').click(function () {
                ...
                if (className != "first first_off") {
                    window.location = "/PurchaseRequest/Navigation/?name=" + $('#btnFirst').val()
            ...
            $('#btnPre').click(function () {
                ...
                if (className != "prev prev_off") {
                    window.location = "/PurchaseRequest/Navigation/?name=" + $('#btnPre').val()
                    + "&id=" + $('#ID').val() + "&Page=Detail";
                }
            });
            $('#btnNext').click(function () {
                ...
                if (className != "next next_off") {
                    window.location = "/PurchaseRequest/Navigation/?name=" + $('#btnNext').val()
                    + "&id=" + $('#ID').val() + "&Page=Detail";
                }
            });
            $('#btnLast').click(function () {
                ...
                if (className != "last last_off") {
                    ...
                    window.location = "/PurchaseRequest/Navigation/?name=" + $('#btnLast').val()
                + "&id=" + $('#ID').val() + "&Page=Detail";
                }
            });
            /*---------------------End navigation*/
            $("#<%=CommonDataKey.PR_ROLE%>").change(function () {
                navigateWithReferrer("/PurchaseRequest/ChangeRole/?RoleId=" + $(this).val());
            });
            $("#addForm").validate({
                debug: false,
                errorElement: "span",
