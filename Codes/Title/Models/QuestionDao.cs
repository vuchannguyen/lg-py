...
namespace CRM.Models
{
    ...
    public class QuestionDao : BaseDao
    {
        ...
        /// <summary>
        /// Get list of questions merge with listening topics and comprehension paragraphs
        /// </summary>
        /// <returns></returns>
        public List<LOT_Question> GetListMergeWithTopicAndParagraph(string name, string sectionID)
        {
            List<LOT_Question> questionList=dbContext.LOT_Questions
                .Where(p => (p.DeleteFlag == false)).ToList<LOT_Question>();
            List<LOT_ListeningTopic> topicList = dbContext.LOT_ListeningTopics
                .Where(p=>p.DeleteFlag==false).ToList<LOT_ListeningTopic>();
            List<LOT_ComprehensionParagraph> paragraphList = dbContext.LOT_ComprehensionParagraphs
                .Where(p => p.DeleteFlag == false).ToList<LOT_ComprehensionParagraph>();
            foreach(LOT_ListeningTopic topic in topicList)
            {
                questionList.Add(new LOT_Question() { 
                    ID=topic.ID, 
                    QuestionContent=topic.TopicName ,
                    LOT_Section = dbContext.LOT_Sections.Single(p=>p.ID == Constants.LOT_LISTENING_TOPIC_ID ),
                    SectionID = Constants.LOT_LISTENING_TOPIC_ID
                });
            }
            foreach (LOT_ComprehensionParagraph paragraph in paragraphList)
            {
                questionList.Add(new LOT_Question()
                {
                    ID = paragraph.ID,
                    QuestionContent = paragraph.ParagraphContent,
                    LOT_Section = dbContext.LOT_Sections
                        .Single(p => p.ID == Constants.LOT_COMPREHENSION_SKILL_ID),
            ...
            if (!string.IsNullOrEmpty(name))
            {
                questionList = questionList
                    .Where(p => p.QuestionContent.Trim().ToLower().Contains(name.Trim().ToLower()))
                    .ToList(); ;
            }
            if (!string.IsNullOrEmpty(sectionID))
            {
                questionList = questionList
                    .Where(p => p.SectionID == int.Parse(sectionID)).ToList();
            }
            return questionList;
        }
        /// <summary>
        ...
        /// <param name="numberOfQuestions"></param>
        /// <returns></returns>
        public List<LOT_Question> GetQuestionListRandomBySectionListeningSkillID(int numberOfQuestions, int candidateExamID)
        {
            try
            {
        ...
        /// <returns></returns>
        public List<CandidateAnswerQuestion> GetQuestionListFromCandidateAnswer(int candidateExamID, int examQuestionSectionID)
        {
