...
namespace CRM.Models
{
    public class CandidateExamDao : BaseDao
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public LOT_Candidate_Exam GetByID(int id)
        {
        ...
        public double? GetMarkOfSection(int candidateExamId, int sectionId)
        {
            if (candidateExamId == 0 || sectionId == 0)
                return null;
            return dbContext.GetScoreOfSection(candidateExamId, sectionId, Constants.LOT_WRITING_SKILL_ID, Constants.LOT_VERBAL_SKILL_ID);
        }
