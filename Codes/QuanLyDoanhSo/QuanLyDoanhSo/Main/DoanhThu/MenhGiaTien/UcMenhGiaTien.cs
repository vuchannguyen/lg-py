...
using Library;
using Weedon.MenhGiaTien;
using DTO;
using BUS;

namespace Weedon
{
    public partial class UcMenhGiaTien : UserControl
    {
        ...
        private void UcMenhGiaTien_Load(object sender, EventArgs e)
        {
            this.Visible = false;
            ...
            string sortColumn, string sortOrder, int page)
        {
            ...
            int total = MenhGiaTienBus.GetCount(text);
            ...
            List<DTO.MenhGiaTien> list = MenhGiaTienBus.GetList(text,
                sortColumn, sortOrder, row * (page - 1), row);

            CommonFunc.ClearlvItem(lvThongTin);

            foreach (DTO.MenhGiaTien data in list)
            {
        ...
        private void pbSua_Click(object sender, EventArgs e)
        {
            ...
            uc = new UcInfo(MenhGiaTienBus.GetById(ConvertUtil.ConvertToInt(id)));
            uc.Disposed += new EventHandler(uc_Disposed);
            this.Controls.Add(uc);
        }
