...
namespace Weedon
{
    public partial class UcBanHang : UserControl
    {
        ...
        private void Init()
        {
            ...
            tbSearch.Text = Constant.SEARCH_BANHANG_TIP;
        ...
        {
            if (text == Constant.SEARCH_BANHANG_TIP)
            {
                text = string.Empty;
            }

            int total = BanHangBus.GetCount(text);
            ...
            List<DTO.BanHang> list = BanHangBus.GetList(text,
                sortColumn, sortOrder, row * (page - 1), row);

            CommonFunc.ClearlvItem(lvThongTin);

            foreach (DTO.BanHang data in list)
            {
                ...
                lvi.SubItems.Add(data.User.To == null ? "0" : data.User.To.ToString());
        ...
        {
            DTO.BanHang data = BanHangBus.GetById(id);
        ...
        private void RefreshLvEx(string text)
        {
            ...
            //        {
            //            lvi.Text = list[i].MaBanHang;
            //        }
            //        else
            //        {
            //            lvi.SubItems.Add(list[i].MaBanHang);
            //        }
            //    }
            ...
            //    if (lvEx.Columns[colNum].Visible)
            //    {
            //        lvi.SubItems.Add(list[i].BanHangGroup.Ten);
            //    }
        ...
        private void pbSua_Click(object sender, EventArgs e)
        {
            ...
            uc = new UcInfo(BanHangBus.GetById(id));
            uc.Disposed += new EventHandler(uc_Disposed);
            this.Controls.Add(uc);
        }
        ...
        private void lvThongTin_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                if (lvThongTin.SelectedItems.Count > 0)
                {
                    ...
                    //UserControl uc = new UcDetail(BanHangBus.GetById(id));
                    //this.Controls.Add(uc);
                }
            }
        }
        ...
        {
            if (tbSearch.Text == Constant.SEARCH_BANHANG_TIP)
            {
                tbSearch.Text = string.Empty;
            }
        }
