...
namespace Weedon
{
    public partial class UcNhapHang : UserControl
    {
        ...
        private List<DTO.NhapHang> listData;
        ...
        private void Init()
        {
            ...
            listData = new List<DTO.NhapHang>();

            RefreshData(string.Empty, 0, 0, dtpFilter.Value);
        }
        ...
        private void RefreshData(string text, int idUser, int idNguonCungCap, DateTime date)
        {
            dgvThongTin.Rows.Clear();

            if (NhapHangBus.GetCount(text, idUser, idNguonCungCap, date) > 0)
            {
                List<DTO.NhapHang> list = NhapHangBus.GetList(text, idUser, idNguonCungCap, date,
        ...
        private void InsertData()
        {
            foreach (DataGridViewRow row in dgvThongTin.Rows)
            {
                ...
                if (NhapHangBus.Insert(data, FormMain.user))
                {
                    //this.Dispose();
                }
                else
        ...
        private void UpdateData()
        {
            foreach (DataGridViewRow row in dgvThongTin.Rows)
            {
                DTO.NhapHang data = NhapHangBus.GetById(ConvertUtil.ConvertToInt(row.Cells[colId.Name].Value));
