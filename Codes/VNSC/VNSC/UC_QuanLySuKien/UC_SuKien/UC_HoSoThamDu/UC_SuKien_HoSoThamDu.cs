...
namespace VNSC
{
    public partial class UC_SuKien_HoSoThamDu : UserControl
    {
        ...
        private List<string> list_DoiTuong;
        ...
        private void UC_SuKien_HoSoThamDu_Load(object sender, EventArgs e)
        {
            ...
            list_DoiTuong = new List<string>();
            list_TrachVu = new List<string>();

            cbID_LLCN.Items.Add(" ");
            LayDSID_ComboBox(cbID_LLCN);

            if (!LayDSDoiTuong_ComboBox(cbDoiTuong_LLCN))
            {
                this.Visible = false;
                Form_Notice frm = new Form_Notice("Chưa khởi tạo Đối tượng!", false);
                return;
            }
            else
            {
            ...
            pnLyLich.Location = SubFuntion.SetCenterLocation(this.Size, pnLyLich.Size);

            gbLyLichCaNhan.Size = new System.Drawing.Size(550, 545);
            gbLyLichCaNhan.Location = SubFuntion.SetWidthCenter(pnLyLich.Size, gbLyLichCaNhan.Size, 0);

            gbLyLichHuongDao.Size = new System.Drawing.Size(550, 545);
            gbLyLichHuongDao.Location = SubFuntion.SetWidthCenter(pnLyLich.Size, gbLyLichHuongDao.Size, 0);

            gbNgheNghiep_KiNang.Size = new System.Drawing.Size(550, 545);
            gbNgheNghiep_KiNang.Location = SubFuntion.SetWidthCenter(pnLyLich.Size, gbNgheNghiep_KiNang.Size, 0);

            gbHuanLuyen.Size = new System.Drawing.Size(550, 545);
            gbHuanLuyen.Location = SubFuntion.SetWidthCenter(pnLyLich.Size, gbHuanLuyen.Size, 0);

            pn_gbHuanLuyen.Size = new System.Drawing.Size(540, 430);
            pn_gbHuanLuyen.Location = SubFuntion.SetWidthCenter(gbHuanLuyen.Size, pn_gbHuanLuyen.Size, 50);
        ...
        private string NewMa()
        {
            if (list_dto.Count > 0)
            {
                sMaSuKien_HoSo = SubFuntion.ThemMa4So(int.Parse(sMaSuKien_HoSo.Substring(0, 4))) + "SK" + iMaSuKien.ToString();
        ...
        private bool LayDSID_ComboBox(ComboBox cb)
        {
            List<ID> list_Temp = ID_BUS.LayDSID();
        ...
        private bool LayDSDoiTuong_ComboBox(ComboBox cb)
        {
            List<DoiTuong> list_Temp = DoiTuong_BUS.LayDSDoiTuong();
            if (list_Temp.Count > 0)
            {
                list_DoiTuong.Clear();
                cb.Items.Clear();
                for (int i = 0; i < list_Temp.Count; i++)
                {
                    list_DoiTuong.Add(list_Temp[i].Ma);
        ...
        private bool LayDSTrachVuTheoMaDoiTuong_ComboBox(ComboBox cb, string sMaDoiTuong)
        {
            List<TrachVu> list_Temp = TrachVu_BUS.TraCuuDSTrachVuTheoMaDoiTuong(sMaDoiTuong);
            if (list_Temp != null)
            {
                list_TrachVu.Clear();
                cb.Items.Clear();
            ...
            {
                Form_Notice frm = new Form_Notice("Kiểm tra Đối Tượng bị trùng!", false);
                return false;
            }
        }
        ...
        private void setSuKien_HoSoTheoMaSuKien_HoSo(string sMa)
        {
            ...
            if (dto_SuKien_HoSo.MaID == null)
            {
                cbID_LLCN.SelectedIndex = 0;
            }
            else
            {
                cbID_LLCN.Text = ID_BUS.TraCuuIDTheoMa((int)dto_SuKien_HoSo.MaID).DienGiai;
            }
            cbDoiTuong_LLCN.Text = DoiTuong_BUS.TraCuuDoiTuongTheoMa(dto_SuKien_HoSo.MaDoiTuong).Ten;
        ...
        private void setSuKien_HoSoTheoMaHoSo(string sMa)
        {
            ...
            if (dto_HoSo.MaID == null)
            {
                cbID_LLCN.SelectedIndex = 0;
            }
            else
            {
                cbID_LLCN.Text = ID_BUS.TraCuuIDTheoMa((int)dto_HoSo.MaID).DienGiai;
            }
            cbDoiTuong_LLCN.Text = DoiTuong_BUS.TraCuuDoiTuongTheoMa(dto_HoSo.MaDoiTuong).Ten;
        ...
        private void cbID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (cbID_LLCN.SelectedIndex > 0)
                {
                    tbMa_LLCN.Text = sMaSuKien_HoSo + ID_BUS.TraCuuIDTheoMa(list_ID[cbID_LLCN.SelectedIndex - 1]).ID1;
                }
                else
                {
                    tbMa_LLCN.Text = sMaSuKien_HoSo;
        ...
        private void cbDoiTuong_LLCN_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbDoiTuong_LLCN.Text.Length > 0)
            {
                cbTrachVu_LLCN.Items.Clear();
                if (!LayDSTrachVuTheoMaDoiTuong_ComboBox(cbTrachVu_LLCN, list_DoiTuong[cbDoiTuong_LLCN.SelectedIndex]))
                {
                    frm_Notice = new Form_Notice("Chưa có Trách Vụ trong Đối Tượng này!", false);
                }
                else
                {
        ...
        private void NewSuKien_HoSo()
        {
            ...
            if (cbID_LLCN.SelectedIndex > 0)
            {
                dto_SuKien_HoSo.MaID = list_ID[cbID_LLCN.SelectedIndex - 1];
            }

            dto_SuKien_HoSo.MaDoiTuong = list_DoiTuong[cbDoiTuong_LLCN.SelectedIndex];
        ...
        private void pbBrowse_Click(object sender, EventArgs e)
        {
            ...
            if (sPath != null)
            {
                ...
                if (imgAvatar.Width >= 90 && imgAvatar.Height >= 120)
                {
                    ...
                    point_Pic = new Point(imgZoom.Width / 2 - 45, imgZoom.Height / 2 - 60);
        ...
        private void pbAvatar_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            if (e.Delta < 0)
            {
                if (iZoom - 100 > 100)
                {
                    ...
                    if (imgZoom.Width >= 90 && imgZoom.Height >= 120)
                    {
                        size_Pic.Width = imgZoom.Width;
                        size_Pic.Height = imgZoom.Height;
                        point_PicBound = Image_Function.setPicBound(point_PicBound, size_Pic, size_PicRec);
