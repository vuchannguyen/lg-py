...
namespace CRM.Models
{
    public class UserAdminDao : BaseDao
    {
        ...
        {
            List<sp_GetListItHelpDeskResult> lstAdmin = dbContext.sp_GetListItHelpDesk(null, moduleId, permissionid).ToList();
            List<sp_GetListItHelpDeskResult> lstTmp = new List<sp_GetListItHelpDeskResult>();
            
            string sSufMail = Constants.LOGIGEAR_EMAIL_DOMAIN;
            foreach (var admin in lstAdmin)
            {
                var employee = (from emp in dbContext.Employees
                             where emp.OfficeEmail == admin.Name + sSufMail
                             select emp).FirstOrDefault();
                //if (!String.IsNullOrEmpty(employee.ID.ToString()))
                if(employee != null)
                {
                    //admin.ID = Int32.Parse(empId);
                    sp_GetListItHelpDeskResult a = new sp_GetListItHelpDeskResult();
                    a.ID = ConvertUtil.ConvertToInt(employee.ID);
                    a.Name = employee.FirstName + " " + employee.MiddleName + " " + employee.LastName;
                    lstTmp.Add(a);
                }
            }
            return lstTmp;//lstAdmin;
