...
namespace CRM.Models
{
    ...
    public class PurchaseRequestDao :BaseDao
    {
        ...
        public CRM_MasterData GetMasterDataById(string masterID)
        {
            return dbContext.CRM_MasterDatas.Where(p => p.ID == masterID).FirstOrDefault<CRM_MasterData>();
        }
        /// <summary>
        ...
        public sp_GetPurchaseRequestResult GetPurchaseRequestByID(string id)
        {
            ...
            return dbContext.sp_GetPurchaseRequest(id, 0, 0, 0, 0, null, 0, 0, 0, 0, null, null, "", "", null, null)
                .Where(p => p.ID.ToString() == id).FirstOrDefault<sp_GetPurchaseRequestResult>(); ;
        }
        ...
        /// <summary>
        /// Update Exchange Rate in CRM_MasterData
        /// </summary>
        /// <param name="exchangeRate"></param>
        public void UpdateExchangeRate(string exchangeRate)
        {
            CRM_MasterData objMasterDb = GetMasterDataById(Constants.EXCHANGE_RATE_KEYWORD);
            if (objMasterDb != null)
            {
                objMasterDb.Value = exchangeRate;
                dbContext.SubmitChanges();
            }
        }
        /// <summary>
        ...
        public List<sp_GetPurchaseRequestResult> GetList(string text,int department, int subdepartment,
            int requestorId, int assignID, string resolutionId, string billableToClient, int loginId, int loginRole, 
            int statusID, bool? isUsPurchasing, string fromDate="",string toDate="")
        {
            ...
            var result = dbContext.sp_GetPurchaseRequest(text, department, subdepartment, requestorId, assignID, resolutionId, statusID,
                ...
                Constants.SEPARATE_INVOLVE_SIGN, fromDate, toDate, isUsPurchasing, billableToClient).ToList();            
            return result;
        }
        ...
        public List<sp_GetPurchaseRequestResult> Sort(List<sp_GetPurchaseRequestResult> list, string sortColumn, string sortOrder)
        {
            ...
            switch (sortColumn)
            {
                ...
                case "Justification":
                    ...
                    break;
                case "BillableToClient":
                    list.Sort(
                         delegate(sp_GetPurchaseRequestResult m1, sp_GetPurchaseRequestResult m2)
                         { return m1.BillableToClient.CompareTo(m2.BillableToClient) * order; });
                    break;
            }
        ...
        /// <summary>
        /// Get List Billable To Client
        /// </summary>
        /// <returns></returns>
        public List<sp_GetListBillableResult> GetBillableToClient()
        {
            return dbContext.sp_GetListBillable().ToList < sp_GetListBillableResult>();
        }

        internal List<sp_GetPurchaseRequestResult> GetList(string textSearch, int department, int sub_Department, int requestorId, int assignID, string resolutionId, int p, int p_2, int p_3, bool p_4)
        {
