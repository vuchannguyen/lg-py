...
namespace Weedon
{
    public partial class UcLoaiTien: UserControl
    {
        ...
        private void UcLoaiTien_Load(object sender, EventArgs e)
        {
            this.Visible = false;
            ...
            tbSearch.Text = Constant.SEARCH_LOAITIEN_TIP;
        ...
        {
            if (text == Constant.SEARCH_LOAITIEN_TIP)
            {
                text = string.Empty;
            }

            int total = LoaiTienBus.GetCount(text);
            ...
            List<DTO.LoaiTien> list = LoaiTienBus.GetList(text,
                sortColumn, sortOrder, row * (page - 1), row);

            CommonFunc.ClearlvItem(lvThongTin);

            foreach (DTO.LoaiTien data in list)
            {
        ...
        private void pbSua_Click(object sender, EventArgs e)
        {
            ...
            uc = new UcInfo(LoaiTienBus.GetById(ConvertUtil.ConvertToInt(id)));
            uc.Disposed += new EventHandler(uc_Disposed);
            this.Controls.Add(uc);
        }
        ...
        private void lvThongTin_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                if (lvThongTin.SelectedItems.Count > 0)
                {
                    ...
                    //UserControl uc = new UcDetail(LoaiTienBus.GetById(id));
                    //this.Controls.Add(uc);
                }
            }
        }
        ...
        {
            if (tbSearch.Text == Constant.SEARCH_LOAITIEN_TIP)
            {
                tbSearch.Text = string.Empty;
            }
        }
