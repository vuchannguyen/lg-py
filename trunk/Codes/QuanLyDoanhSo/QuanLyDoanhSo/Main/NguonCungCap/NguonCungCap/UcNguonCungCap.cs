...
using Library;
using Weedon.NguonCungCap;
using DTO;
using BUS;

namespace Weedon
{
    public partial class UcNguonCungCap : UserControl
    {
        ...
        private void UcNguonCungCap_Load(object sender, EventArgs e)
        {
            this.Visible = false;
            LoadResource();
            ...
            string sortColumn, string sortOrder, int page)
        {
            ...
            int total = NguonCungCapBus.GetCount(text);
            ...
            List<DTO.NguonCungCap> list = NguonCungCapBus.GetList(text,
                sortColumn, sortOrder, row * (page - 1), row);

            CommonFunc.ClearlvItem(lvThongTin);

            foreach (DTO.NguonCungCap data in list)
            {
                ...
                lvi.SubItems.Add(data.MaSoThue);
        ...
        private void pbXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(Constant.MESSAGE_DELETE_CONFIRM, Constant.CAPTION_CONFIRMATION, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                if (NguonCungCapBus.DeleteList(ids))
                {
                    RefreshListView(tbSearch.Text, false,
                        sortColumn, sortOrder, ConvertUtil.ConvertToInt(lbPage.Text));
                    SetStatusButtonPage(ConvertUtil.ConvertToInt(lbPage.Text));
        ...
        private void pbSua_Click(object sender, EventArgs e)
        {
            ...
            uc = new UcInfo(NguonCungCapBus.GetById(id));
            uc.Disposed += new EventHandler(uc_Disposed);
            this.Controls.Add(uc);
        }
        ...
        {
            pbBirthDay.Image = Image.FromFile(ConstantResource.CHUC_NANG_ICON_BITRHDAY_MOUSEOVER);
        }
        ...
        private void lvThongTin_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                if (lvThongTin.SelectedItems.Count > 0)
                {
                    ...
                    //UserControl uc = new UcDetail(NguonCungCapBus.GetById(id));
                    //this.Controls.Add(uc);
                }
            }
        }
