...
namespace QuanLyKinhDoanh.GiaoDich
{
    public partial class UcThanhToan : UserControl
    {
        ...
        private void CalculateCK()
        {
            ...
            {
                lvThongTin.Columns[5].Text = "Điểm CK";
                lbTongCK.Text = "Điểm CK:";
                tbTienCK.Text = (money / Constant.DEFAULT_CHANGE_RATE).ToString(Constant.DEFAULT_FORMAT_MONEY);
            }
            else
            {
                lvThongTin.Columns[5].Text = "Tiền CK";
                lbTongCK.Text = "Tổng CK:";
        ...
        private void AddToBill()
        {
            ...
            if (!isDuplicated)
            {
                ...
                lvi.SubItems.Add(dataSP.MaSanPham + Constant.SYMBOL_LINK_STRING + dataSP.Ten);
        ...
        private void RemoveFromBill()
        {
            if (MessageBox.Show(Constant.MESSAGE_DELETE_CONFIRM, Constant.CAPTION_CONFIRM, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                foreach (ListViewItem lvi in lvThongTin.CheckedItems)
                {
                    ...
                    else if (lvThongTin.Items.Count == 0)
                    {
                        totalDiscount = 0;
                    }
