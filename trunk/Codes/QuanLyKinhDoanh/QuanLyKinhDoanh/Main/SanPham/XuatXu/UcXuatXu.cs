...
using Library;
using QuanLyKinhDoanh.XuatXu;
using DTO;
using BUS;

namespace QuanLyKinhDoanh
{
    public partial class UcXuatXu : UserControl
    {
        ...
        private void UcXuatXu_Load(object sender, EventArgs e)
        {
            this.Visible = false;
            LoadResource();
            ...
            tbSearch.Text = Constant.SEARCH_XUATXU_TIP;
        ...
        {
            if (text == Constant.SEARCH_XUATXU_TIP)
            {
                text = string.Empty;
            }

            int total = XuatXuBus.GetCount(text);
            ...
            List<DTO.XuatXu> list = XuatXuBus.GetList(text,
                sortColumn, sortOrder, row * (page - 1), row);

            CommonFunc.ClearlvItem(lvThongTin);

            foreach (DTO.XuatXu data in list)
            {
        ...
        private void pbSua_Click(object sender, EventArgs e)
        {
            ...
            uc = new UcInfo(XuatXuBus.GetById(id));
            uc.Disposed += new EventHandler(uc_Disposed);
            this.Controls.Add(uc);
        }
        ...
        private void lvThongTin_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                if (lvThongTin.SelectedItems.Count > 0)
                {
                    ...
                    UserControl uc = new UcDetail(XuatXuBus.GetById(id));
                    this.Controls.Add(uc);
                }
            }
        }
        ...
        {
            if (tbSearch.Text == Constant.SEARCH_XUATXU_TIP)
            {
                tbSearch.Text = string.Empty;
            }
        }
