...
namespace CRM.Models
{
    public class AssetCategoryDao : BaseDao
    {
        ...
        private Message Delete(AssetCategory objUI)
        {
            ...
            try
            {
                if (objUI != null)
                {
                    ...
                    List<sp_GetAssetPropertyResult> assProp = new List<sp_GetAssetPropertyResult>();
                    assProp = assPropDao.GetList(string.Empty, objUI.ID);
                    if (objDb != null && assProp.Count == 0)
                    {
        ...
        private bool isDublicateName(AssetCategory objUI)
        {
            ...
            AssetCategory dublicateName = dbContext.AssetCategories.Where(a => a.Name.Equals(objUI.Name)).SingleOrDefault<AssetCategory>();
