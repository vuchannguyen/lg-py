...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        $("#txtKeyword").autocomplete('/Library/GenericHandle/AutoCompleteHandler.ashx/?Page=JR&Action=' + $("#Func").val());
        CRM.onEnterKeyword();
        ...
        $("#btnFilter").click(function () {
            ...
            var urls = '/Common/GetListJRGrid/?text=' + name
                     + '&department=' + $('#Department').val() + '&sub=' + $('#SubDepartment').val() +
                      '&positionId=' + $('#positionId').val() + '&issueDate=' + $('#txtDate').val() + '&func=' + $("#Func").val() + '&request=' + $('#RequestType').val();
            $('#list1').setGridParam({ url: urls
            }).trigger('reloadGrid');
        });
        jQuery("#list1").jqGrid({
            url: '/Common/GetListJRGrid/?Func=' + $("#Func").val(),
            datatype: 'json',
            mtype: 'GET',
            colNames: ["ID", "Req#", 'Approval', 'Candidate', 'Emp ID', 'Department', 'Sub Department', 'Job Title', 'Issue Date','Request', 'Justification'],
            colModel: [
                    { name: 'RealID', index: 'RealID', align: "center", width: 10, hidden: true },
                    { name: 'ID', index: 'ID', align: "center", width: 50, sortable: false },
                    { name: 'Approval', index: 'Approval', align: "center", width: 50, sortable: false },
                    { name: 'Candidate', index: 'Candidate', align: "left", width: 70, sortable: false },
                    { name: 'EmpID', index: 'EmpID', align: "center", width: 50, sortable: false },
                    ...
                    { name: 'FinalTitleName', index: 'FinalTitleName', align: "left", width: 80, sortable: false },
                    { name: 'IssueDate', index: 'IssueDate', align: "center", width: 70, sortable: false },
                    { name: 'Request', index: 'Request', align: "center", width: 70, sortable: false },
