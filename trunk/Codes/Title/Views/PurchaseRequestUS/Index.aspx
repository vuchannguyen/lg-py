...
<asp:Content ID="Content2" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            //Dropdownlist 
            $("#<%=CommonDataKey.PR_ROLE_US%>").change(function () {
                navigateWithReferrer("/PurchaseRequestUS/ChangeRole/?RoleId=" + $(this).val());
            });
            CRM.onEnterKeyword();
            jQuery("#list").jqGrid({
                url: '/PurchaseRequestUS/GetListJQGrid/?purchaseId=' + encodeURIComponent($('#txtKeyword').val())
                ...
                colModel: [
                    ...
                    { name: 'Priority', index: 'Priority', align: "center", hidden: true },
                    { name: 'ID', index: 'ID', align: "center", width: 54, sortable: true },
                    ...
                    { name: 'PurchaseApproval', index: 'PurchaseApproval', align: "center", width: 70, sortable: true },
                ...
                pager: '#pager',
                sortname: '<%= (string)ViewData[Constants.PURCHASE_REQUEST_COLUMN_US]%>',
                sortorder: '<%= (string)ViewData[Constants.PURCHASE_REQUEST_ORDER_US]%>',
                rowNum: '<%= (string)ViewData[Constants.PURCHASE_REQUEST_ROW_COUNT_US]%>',
                page: '<%= (string)ViewData[Constants.PURCHASE_REQUEST_PAGE_INDEX_US]%>',
                ...
                loadComplete: function () {
                    ...
                    ShowTooltip($("a.justification"), $("#shareit-box"), "/PurchaseRequestUS/JustificationTooltip");
                }
            });
            $("#btnDelete").click(function () {
                CRM.deleteList('#list', 'RealID', '/PurchaseRequestUS/DeleteList');
            });
            $("#btnRefresh").click(function () {
                window.location = "/PurchaseRequestUS/Refresh";
            });
            $('#btnAddNew').click(function () {
                window.location = '/PurchaseRequestUS/Create';
            });
            $('#btnReport').click(function () {
                window.location = '/PurchaseRequestUS/Report';
            });
            $("#btnExport").click(function () {
                ...
                if (numberRow <= 0) {
                    ...
                    window.location = '/PurchaseRequestUS/ExportToExcel/?purchaseId=' + encodeURIComponent($('#txtKeyword').val())
                     + '&department=' + $('#Department').val() + '&subdepartment=' + $('#SubDepartment').val()
            ...
            $("#btnFilter").click(function () {
                ...
                $('#list').setGridParam({ url: '/PurchaseRequestUS/GetListJQGrid/?purchaseId=' + name
                     + '&department=' + $('#Department').val() + '&subdepartment=' + $('#SubDepartment').val()
                     + '&requestorId=' + $('#requestorId').val() + '&assignId=' + $('#assignId').val() + '&resolutionId=' + $('#resolutionId').val()
                }).trigger('reloadGrid');
            });
...
<asp:Content ID="Content5" ContentPlaceHolderID="LoginRoles" runat="server">
    <% if (ViewData[CommonDataKey.PR_ROLE_US] != null)
       {%>
    <span class="bold">Login As: </span>
    <%=Html.DropDownList(CommonDataKey.PR_ROLE_US, null, new { @style = "width:180px" })%>
    <%} %>
</asp:Content>
<asp:Content ID="Content6" ContentPlaceHolderID="MainContent" runat="server">
            ...
            <tr>
                <td>
                    ...
                    <%                       
                        ...
                        if (ViewData.Model == Constants.PR_REQUESTOR_ID_US)
                        {%>
    ...
    <div id="cfilter">
        <% Response.Write(Html.Hidden("IsViewAll", ViewData[CommonDataKey.PR_IS_VIEW_ALL_US])); %>
        <% Response.Write(Html.Hidden("LoginId", ViewData[CommonDataKey.PR_USER_LOGIN_ID_US])); %>
        
        <% Response.Write(Html.Hidden("LoginName", ViewData[CommonDataKey.PR_USER_LOGIN_NAME_US])); %>
        <table>
            <tr>
                <td>
                    <input type="text" id="txtKeyword" maxlength="100" style="width: 150px" value="<%= (string)ViewData[Constants.PURCHASE_REQUEST_KEYWORD_US]  %>"
                        onfocus="ShowOnFocus(this,'<%= Constants.PURCHASE_REQUEST  %>')" onblur="ShowOnBlur(this,'<%= Constants.PURCHASE_REQUEST  %>')" autocomplete="off" />
                </td>
                <td>
                    <%=Html.DropDownList("Department", ViewData[Constants.PURCHASE_REQUEST_DEPARTMENT_US] as SelectList, Constants.FIRST_ITEM_DEPARTMENT)%>
                </td>
                <td>
                    <%=Html.DropDownList("SubDepartment", ViewData[Constants.PURCHASE_REQUEST_SUB_DEPARTMENT_US] as SelectList, Constants.FIRST_ITEM_SUB_DEPARTMENT)%>
                </td>
                <td>
                    <%=Html.DropDownList("requestorId", ViewData[Constants.PURCHASE_REQUEST_REQUESTOR_ID_US] as SelectList, Constants.PURCHASE_REQUEST_REQUESTOR_FIRST_ITEM)%>
                </td>

                <td>
                    <%=Html.DropDownList("assignId", ViewData[Constants.PURCHASE_REQUEST_ASSIGN_ID_US] as SelectList, Constants.PURCHASE_REQUEST_ASSIGN_FIRST_ITEM)%>
                </td>
                <td>
                    <%=Html.DropDownList("resolutionId", ViewData[Constants.PURCHASE_REQUEST_RESOLUTION_ID_US] as SelectList, Constants.PURCHASE_REQUEST_STATUS_FIRST_ITEM)%>
                </td>
                <td>
