...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <%=TempData["Message"]%>
    <%using (Html.BeginForm("SetupApproval", "PurchaseRequestUS", FormMethod.Post, new { id = "setupForm" }))
      {
          ...
          Response.Write(Html.Hidden("hidValue", ViewData[CommonDataKey.PR_LIST_COUNT_US]));
    ...
    ((sp_GetPurchaseRequestResult)ViewData.Model).ExpectedDate.Value.ToString(Constants.DATETIME_FORMAT_VIEW) : "")%>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td align="left" class="style1" style="color: #0066CC; font-size: 1.4em">
                    Item(s)
                </td>
        ...
        <div class="form">
            <table border="0" cellpadding="0" cellspacing="0" width="100%" class="view_item">
                ...
                <% if (ViewData[CommonDataKey.LIST_PURCHASE_ITEM_US] != null) %>
                <%  {
                        List<PurchaseItem> items = (List<PurchaseItem>)ViewData[CommonDataKey.LIST_PURCHASE_ITEM_US];
        ...
        <div class="setup_approval" style="margin-top: 130px">
            ...
            <%
                ...
                int max = Convert.ToInt32(ViewData[CommonDataKey.PR_MAX_APPROVAL_US]);
                int listCount = Convert.ToInt32(ViewData[CommonDataKey.PR_LIST_COUNT_US]);
                List<sp_GetListApprovalAssignResult> listAppro = (List<sp_GetListApprovalAssignResult>)ViewData[CommonDataKey.LIST_APPROVAL_US];
                for (int i = 1; i <= max; i++)
                {
...
if (approval != null && i <= listCount)
    listUser = new SelectList(adminList, "UserAdminId", "UserName", approval != null ? approval.UserAdminId : 0);%>
                        <td class="header" style="width: 175px">
                            <% if (approval != null && i <= listCount)
                               { %>
                            <%=Html.DropDownList("Approval" + i.ToString(), listApp, Constants.FIRST_ITEM_CHOOSE, new { @style = "width:150px" })%>
                            <% }
                               else
                               { %>
                            <%=Html.DropDownList("Approval" + i.ToString(), ViewData[CommonDataKey.PR_ROLE_US] as SelectList, Constants.FIRST_ITEM_CHOOSE, new { @style = "width:150px" })%>
                            <%} %>
                        </td>
                        <td class="header" style="width: 175px">
                            <%  
                            ...
                            <% 
                                    ...
                                    <%=Html.DropDownList(str, temp, Constants.FIRST_ITEM_CHOOSE, new { @style = "width:150px" })%>
                             <% }
                                else
                                { %>
                                    <%=Html.DropDownList(str, listUser, new { @style = "width:150px" })%>
                             <% } %>
                        </td>
                    </tr>
                </table>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="form">
                ...
                <td colspan="2">
                    <table cellspacing="0" cellpadding="0" border="0"  class="view">
                        <tr>
                            ...
                            <td class="input last">
                                <%=Html.DropDownList(CommonDataKey.PR_FORWARD_US, null, Constants.FIRST_ITEM, new { @style = "width:200px" })%>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td class="input">
                                <%=Html.DropDownList(CommonDataKey.LIST_US_ACCOUNTING_US, null, Constants.FIRST_ITEM, new { @style = "width:200px" })%>
                            </td>
                        </tr>
                        <script type="text/javascript">
                            $(document).ready(function () {
                                $("#<%=CommonDataKey.LIST_US_ACCOUNTING_US%>").rules("add", "required");
                            });
                        </script>
                        <%} %>--%>
                    </table>
            ...
            <tr>
                <td colspan="2" align="center">
                    ...
                    <input type="button" class="cancel" value="" id="btnCancel" onclick="window.location = '/PurchaseRequestUS'" />
                </td>
            </tr>
        </table>
    </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="HeadContent" runat="server">
    <script type="text/javascript">
        var index =parseInt('<%=ViewData[CommonDataKey.PR_LIST_COUNT_US] %>');
        var indexSubmit = 0;
        function AddRow() {
            var max = parseInt('<%=Constants.PR_MAX_APPROVAL_US%>');
            if (index < max) {
                index++;
        ...
        function setValidate() {
            $("#<%=CommonDataKey.PR_FORWARD_US%>").rules("add", { required: true });
//            $("#Assign1").rules("add", { required: true });
//            $("#Approval1").rules("add", { required: true });                
            for(var i=1; i<= index;i++)
            {
...
//            }, jQuery.validator.messages.compareRole);       
            ...
            $("#setupForm").validate({
                ...
                submitHandler: function (form) {
                    jQuery.ajax({
                        url: '/PurchaseRequestUS/CheckMultiOfficeStatus',
                        dataType: 'json',
                        data: ({
                            'Assign1': $("#Assign1").val(),
                            'Forward_Us': $("#Forward_Us").val()                    
                            }),
...
<asp:Content ID="Content4" ContentPlaceHolderID="FunctionTitle" runat="server">
    <% sp_GetPurchaseRequestResult request = (sp_GetPurchaseRequestResult)ViewData.Model;
       ...
       funcTitle = CommonFunc.GetCurrentMenu(Request.RawUrl) +
           "<a href='/PurchaseRequestUS/Detail/" + request.ID + "'>" + Constants.PR_REQUEST_PREFIX + request.ID + "</a> » " + PurchaseRequestPageInfo.FuncSetupApproval;
    %>
    <%= funcTitle%>
</asp:Content>
