...
<asp:Content ID="Content2" ContentPlaceHolderID="PageTitle" runat="server">
    <%= TrainingCenterPageInfo.ComName + CommonPageInfo.AppSepChar + TrainingCenterPageInfo.FuncEngCourseAttend + 
        CommonPageInfo.AppSepChar + CommonPageInfo.AppName%>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="HeadContent" runat="server">
<script type="text/javascript">
        function getListTargetUrl() {
            var url = '/TrainingCenterAdmin/GetListEnglishCourseAttend/?' +
                'text=' + $('#txtKeyword').val() +
                '&title=' + $('#TitleId').val() +
        ...
        $(document).ready(function () {            
            ...
            $("#btnRefresh").click(function () {
                window.location = "/TrainingCenterAdmin/RefreshEnglishCourseAttend";
            });
            ...
            $("#btnExport").click(function () {
                ...
                if (numberRow <= 0) {
                    ...
                    window.location = "/TrainingCenterAdmin/ExportEnglishCourseAttend/<%=Constants.TRAINING_CENTER_CLASS_TYPE_ENGLISH %>";
                }
                CRM.completed();
            });
        });
      ...
      <%
        string urlLink = "/TrainingCenterAdmin/GetListEnglishCourseAttend/?"+
        ...
        List<Training_Course> empList = ViewData.Model == null ? null : (List<Training_Course>)ViewData.Model;
            ...
            grid.LoadComplete = "ShowTooltip($('a[class=showTooltip]'), $('#shareit-box'), '/TrainingCenterAdmin/EmpClassTooltip');" +
                                " ShowTooltip($('a[class=empTooltip]'), $('#shareit-box'), '/TrainingCenterAdmin/EmpEnTooltip');"
            ...
            {
                //colNames[i + 5] = CommonFunc.Link(empList[i].CourseId.ToString(), "#", HttpUtility.HtmlEncode(empList[i].Name), true);
                colNames[i + 5] = "<div title=\"" + HttpUtility.HtmlEncode(empList[i].Name) + "\">" + HttpUtility.HtmlEncode(empList[i].CourseId) + "</div>";
                //colNames[i + 5] = HttpUtility.HtmlEncode(empList[i].Name);
            }
           // colNames.ToString();
            string[] colModels = new string[countColumn];
            colModels[0] = "{name: 'ID', index: 'ID', align: 'center', width: 70}";
            colModels[1] = "{name: 'FullName', index: 'FullName', align: 'left', width: 140}";
            colModels[2] = "{name: 'TitleName', index: 'TitleName', align: 'left', width: 150}";
            colModels[3] = "{name: 'DepartmentName', index: 'DepartmentName', left: 'center', width: 150}";
            colModels[4] = "{name: 'ManagerName', index: 'ManagerName', align: 'left', width: 140}";
            for (int i = 0; i < empList.Count; i++)
            {
...
<asp:Content ID="Content5" ContentPlaceHolderID="FunctionTitle" runat="server">
    <%=CommonFunc.GetCurrentMenu(Request.RawUrl)%>English Courses Attendance
