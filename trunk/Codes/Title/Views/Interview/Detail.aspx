...
<script type="text/javascript">
    ...
    $(document).ready(function () {     
            ...
            $('#btnFirst').click(function () {
                ...
                if (className != "first first_off") {
                    window.location = "/Interview/Detail/" + $('#btnFirst').val();
                }
            });
            $(".as-close").attr("style","display:none");

            $('#btnPre').click(function () {
                var className = $('#btnPre').attr('class');
                if (className != "prev prev_off") {
                    window.location = "/Interview/Detail/" + $('#btnPre').val();
                }
            });
            $('#btnNext').click(function () {
                var className = $('#btnNext').attr('class');
                if (className != "next next_off") {
                    window.location = "/Interview/Detail/" + $('#btnNext').val();
                }
            });
            $('#btnLast').click(function () {
                var className = $('#btnLast').attr('class');
                if (className != "last last_off") {
                    window.location = "/Interview/Detail/" + $('#btnLast').val();
                }
            });
            /*---------------------*/         
            $("#btnActive").click(function(){
                $("#divActive").toggleClass('show').toggleClass('hidden');
                $(this).toggleClass('icon minus').toggleClass('icon plus');
            });

            $("#btnOld").click(function(){
                $("#divOld").toggleClass('hidden').toggleClass('show');
    ...
    <% 
        ...
        if (intList == null)
        {
            ...
            List<sp_GetInterviewListResult> InterviewList = interviewDao.GetInterviewList("", 0, 0, "", null, null);
            var finalList = interviewDao.Sort(InterviewList, "ID", "desc");
        ...
        if (intList.Count > 1)
        {
            ...
            //index = intList.IndexOf(intList.Where(p => p.ID == canObj.ID).FirstOrDefault<sp_GetInterviewListResult>());
    ...
    <div class="profile" style="width: 1024px">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="view" width="1024px">
        <tr>
            ...
            <td class="input" style="width: 194px">
                <%
                    if (!string.IsNullOrEmpty(((Candidate)ViewData.Model).CVFile))
                    {
    ...
    <fieldset class="form" style="width: 1008px; background-color: White !important">
        <legend>
            <button type="button" class="icon minus" id="btnActive">
            </button>
            Current Interview </legend>
        <div id="divActive" class="show">
            <%
                int i = 1;
                List<sp_GetInterviewCandidateResult> listInter = (List<sp_GetInterviewCandidateResult>)ViewData["InterviewCandiActive"];
                foreach (sp_GetInterviewCandidateResult inter in listInter)
                {%>
            <fieldset class="form" style="width: 980px;">
                <legend>Round
                    <%=inter.Round + " : " + inter.InterviewDate.Value.ToString(Constants.DATETIME_FORMAT_VIEW) %>
                </legend>
                <table cellpadding="0" cellspacing="0" class="view" style="width: 980px; border: 0px;">
                    <tr>
                        <td class="label" style="width: 120px;">
                            Date
                        </td>
...
if (inter != null && inter.InterviewResultId != null)
{
    ...
    if (result != null)
        Response.Write(result.Name);
                    ...
                    <tr>
                        ...
                        <td class="input">
                            <% =inter.IsSendMailInterviewer != false ? "Yes" : "No" %>
                        </td>
                        <td class="label">
                            Email to Candidate
                        </td>
                        <td class="input">
                            <%=inter.IsSentMailCandidate != false ? "Yes" : "No"%>
                        </td>
                    </tr>
                    <tr>
                    <td class="label">
                    ...
                    <td colspan="5" class="input" style="width: 870px;">
                        <%
                                Response.Write(Html.TextBox("fieldCC" + i, (inter != null ? inter.CClist : ""), new { @style = "width:820px", @readonly = "true", @disabled = "true" }));
                                i++;   
                        %>                        
                    </td>                    
                    </tr>
                    <tr class="last">
                        ...
                        <td colspan="5" class="input" style="width: 870px;">
                            <%
                                Response.Write(Html.TextArea("Comment", inter != null ? inter.Content : "", 2, 1, new { @style = "width:820px", @readonly = "true" }));
                            %>
                        </td>
                    </tr>
                </table>
    ...
    <br />
    <%    List<sp_GetInterviewCandidateResult> listInterOld = (List<sp_GetInterviewCandidateResult>)ViewData["InterviewCandiOld"];
          if (listInterOld.Count > 0)
          { %>
    <fieldset class="form" style="width: 1008px; background-color: White !important">
        <legend>
            <button type="button" class="icon plus" id="btnOld">
            </button>
            &nbsp;Old Interview </legend>
        <div id="divOld" class="hidden">
            <% i = 1;
              foreach (sp_GetInterviewCandidateResult inter in listInterOld)
               {%>
            <fieldset class="form" style="width: 980px;">
                <legend>Round
                    <%=inter.Round + " : " + inter.InterviewDate.Value.ToString(Constants.DATETIME_FORMAT_VIEW) %>
                ...
                <table cellpadding="0" cellspacing="0" class="view" style="width: 980px; border: 0px;">
                            ...
                            <%                     
                                if (inter != null && inter.InterviewFormId != null)
                                {
                                    ...
                                    if (rt != null)
                                    {
                                        ...
                                        if (rt.Code == Constants.INTERVIEW_FORM_CODE + "-1")
                                        {
                                            str = "<a href=\"javascript:void(0);\" onclick=\"CRM.popup('/EForm/DetaiInteviewR1/" + eFormIndex + "','Result preview', 1024)\">" + rt.Name + "</a>";
                                        }
                                        else if (rt.Code == Constants.INTERVIEW_FORM_CODE + "-2")
                                        {
                                            str = "<a href=\"javascript:void(0);\" onclick=\"CRM.popup('/EForm/DetaiInteviewR2/" + eFormIndex + "','Result preview', 1024)\">" + rt.Name + "</a>";
                                            //Response.Write("CRM.popup('" + Html.ActionLink(rt.Name, "Round2", "EForm", new { id = eFormIndex }) + "','Result preview', 1024)");                                
                                        }
                                        else if (rt.Code == Constants.INTERVIEW_FORM_CODE + "-3")
                                        {
                                            str = "<a href=\"javascript:void(0);\" onclick=\"CRM.popup('/EForm/DetaiInteviewR3/" + eFormIndex + "','Result preview', 1024)\">" + rt.Name + "</a>";
                    ...
                    <tr>
                        ...
                        <td class="input">
                            <%  Response.Write(inter.IsSendMailInterviewer != false ? "Yes" : "No"); %>
                        </td>
                        <td class="label">
                            Email to Candidate
                        </td>
                        <td class="input">
                            <% Response.Write(inter.IsSentMailCandidate != false ? "Yes" : "No"); %>
                        </td>
                    </tr>
                    <tr>
                        <td class="label">
                        ...
                        <td colspan="5" class="input" style="width: 870px;">
                            <%
                                    Response.Write(Html.TextBox("fieldCC" + i, (inter != null ? inter.CClist : ""), new { @style = "width:820px", @readonly = "true" }));
                                    i++;
                            %>                        
                        </td>                    
                    </tr>
...
<asp:Content ID="Content2" ContentPlaceHolderID="PageTitle" runat="server">
    <%= HiringCenterPageInfo.MenuName + CommonPageInfo.AppSepChar + HiringCenterPageInfo.FuncInterviewDetails + CommonPageInfo.AppSepChar +  CommonPageInfo.AppName%>
</asp:Content>
...
<%
    ...
    if (can != null)
    {
        ...
        // CommonPageInfo.AppDetailSepChar + can.FirstName + " " + can.MiddleName + " " + can.LastName ;
        funcTitle = CommonFunc.GetCurrentMenu(Request.RawUrl) + can.FirstName + " " + can.MiddleName + " " + can.LastName;
    }
    else
    {
        Response.Redirect("/Interview");
